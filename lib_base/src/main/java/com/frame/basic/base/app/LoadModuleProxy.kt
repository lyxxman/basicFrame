...
class LoadModuleProxy : ApplicationLifecycle {
    ...
    override fun initByFrontDesk(): InitDepend {
        ...
        mLoader.forEach {
            it.initByFrontDesk()?.let { depends ->
                mainThreadDepends.addAll(depends.mainThreadDepends)
                workerThreadDepends.addAll(depends.workerThreadDepends)
            }
        }
