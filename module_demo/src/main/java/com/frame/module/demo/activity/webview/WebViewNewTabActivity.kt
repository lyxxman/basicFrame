...
import com.frame.module.demo.databinding.DemoActivityWebviewNewTabBinding
 ...
 * @Version:
 */
class WebViewNewTabActivity :
    CommonBaseActivity<DemoActivityWebviewNewTabBinding, WebViewNewTabActivity.WebViewVM>(),
    WebViewPlugin {
    override fun DemoActivityWebviewNewTabBinding.initView() {

    }

    override fun DemoActivityWebviewNewTabBinding.initListener() {
        mBindingVM.url.observe(this@WebViewNewTabActivity) {
            webView.loadUrl(it)
        }
    }

    override fun title() = mBindingVM.url.value?:""
    override val mBindingVM: WebViewVM by viewModels()
    ...
    override fun getErrorContainerView(error: Int, msg: String?): View {
        val vb =  LayoutInflater.from(this).inflate(R.layout.common_demo_error_container_view, null, false).viewBindings<CommonDemoErrorContainerViewBinding>()
            .apply {
